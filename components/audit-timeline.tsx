...
function readIdentifierValue(identifiers: string[] | undefined, prefix: string): string | null {
...
function getActionTitle(action: LogisticsAction): string {
  return readIdentifierValue(action.otherIdentifiers, 'actionLabel:') ?? 'Supervisor response';
}
...
function buildTimelineEntries(
  ...
  return [...eventEntries, ...actionEntries, ...loadingEntries].sort((left, right) => {
  ...
  if (entries.length === 0) {
    return (
      <div className="flex min-h-48 items-center justify-center rounded-lg border border-dashed border-border bg-background/40 px-4">
        <p className="text-sm text-muted-foreground">No audit events, actions, or loadings recorded.</p>
      </div>
    );
  }
        ...
        return (
          ...
          >
            <div className="flex min-w-0 flex-col gap-1 pt-1">
              ...
              <p className="text-sm font-semibold text-foreground">{formatDemoClock(entry.timestamp)} UTC</p>
              <p className="text-xs text-muted-foreground">{formatWallClock(entry.timestamp)}</p>
            </div>
